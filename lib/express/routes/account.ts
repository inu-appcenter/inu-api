...
import {NotEnoughParameters} from '../../common/error/errors';

export default express.Router().get('/account/status', async (req, res) => {
  const {studentId, password} = req.query;

  assert(studentId && typeof studentId === 'string', NotEnoughParameters());
  assert(password && typeof password === 'string', NotEnoughParameters());
